...
| **ts-multinet** *(MVP)* | Several tailnets transparently on one host at once | Reach services across many tailnets simultaneously — the thing `tailscaled` can't do. Runs on the host (browser-login nodes, selected resources in `/etc/hosts`) or in a container. See [docs](./docs/ts-multinet.md) |
