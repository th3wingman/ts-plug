...
| Auth-key enrollment | per-tailnet `auth_key` (tagged/automation nodes, no browser): `add` accepts it, `login <tailnet> <authkey>` applies it (restarts a NeedsLogin node), Settings field; `GET /config` redacts it and mutations reload from disk so updates can't wipe it; ambient TS_AUTHKEY envs still rejected | *this branch* |
| Tailnet on/off toggle | the `enabled` flag exposed without config edits: `POST /tailnet/{name}/enabled`, CLI `enable`/`disable`, Settings switch. Disabled tailnets stay visible in status (`disabled` vs `parked`); the patcher treats `enabled` as a mutable member, so comments survive off/on | *this branch* |
